...
attributes(diffMean) <- list(summary="mean",
...
attributes(diffMedian) <- list(summary="median",
														comparison="difference")
...
attributes(ratioMean) <- list(summary="mean",
														comparison="ratio")
...
attributes(ratioMedian) <- list(summary="median",
														comparison="ratio")
...
#' fastPerm(x, y, testStat = diffMean)
	...
	if (attributes(testStat)$comparison == "ratio" & 
			(min(x,y)<0)) {
	...
	# if (printMstop) {
		# if (attributes(testStat)$comparison == "ratio") {
			# mStop <- mStopRatio(x, y, B, plot = plotMstop)
		# } else if (attributes(testStat)$comparison == "difference") {
			# mStop <- mStopDiff(x, y, B, plot = plotMstop)
		# }
		
	# writeLines(paste("Expected mStop = ", mStop, "\n\n", sep = ""))
